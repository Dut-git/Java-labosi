...
public class Glavna {
    ...
    public static void main(String[] args) {
        ...
        for (int k = 0; k < brojObrazovnihUstanova; k++) {
            ...
            do {
                try {
                    ...
                    sc.nextLine();
                    if (brojIspita < MIN_ISPITA) {
                        throw new NeispravanPodatakException("Morate unijeti minimalno " + MIN_ISPITA + " ispita!");
                    }
                } catch (NeispravanPodatakException exception) {
                    ponoviPetlju = true;
                    logger.error("Pogresno unesen podatak: ", exception);
            ...
            long pocetak;
            long kraj;
            System.out.println("Ispisivanje studenata sa for petljom:");
            ...
            System.out.println("Potrebno vrijeme: " + (kraj - pocetak) + "ms\n");

            System.out.println("Ispisivanje studenata sa lambda izrazom:");
            pocetak = System.currentTimeMillis();
            ...
            System.out.println("Ispisivanje studenata s ocjenom izvrstan sa for petljom:");
            pocetak = System.currentTimeMillis();
            for (int i = 0; i < brojIspita; i++) {
            ...
            System.out.println("Ispisivanje studenata s ocjenom izvrstan sa lambda izrazom:");
            pocetak = System.currentTimeMillis();
            ...
            if (sveuciliste.dohvatiObrazovnuUstanovu(k) instanceof FakultetRacunarstva racunarstvo) {
                ispitiStudenta = racunarstvo.filtrirajIspitePoStudentu(ispiti, studenti.get(i));
            }
            if (sveuciliste.dohvatiObrazovnuUstanovu(k) instanceof VeleucilisteJave racunarstvo) {
                ispitiStudenta = racunarstvo.filtrirajIspitePoStudentu(ispiti, studenti.get(i));
            }
            if (sveuciliste.dohvatiObrazovnuUstanovu(k) instanceof FakultetRacunarstva racunarstvo) {
            ...
            if (sveuciliste.dohvatiObrazovnuUstanovu(k) instanceof VeleucilisteJave java) {
                BigDecimal konacnaOcjena;
                if(!studenti.get(i).isNegativanStudent()) {
        ...
        sveuciliste.getObrazovneUstanove().stream().sorted((o1, o2) ->
        {
        ...
        }).forEach(ustanova -> System.out.println("Obrazovna ustanova " + ustanova.getNazivUstanove() + " ima " + ustanova.getStudenti().size() + " studenata."));
        sc.close();
}
}
