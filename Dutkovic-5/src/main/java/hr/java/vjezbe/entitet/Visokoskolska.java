...
public interface Visokoskolska{
    ...
    default List<Ispit> filtrirajIspitePoStudentu(List<Ispit> ispiti, Student student){
        List<Ispit> ispiti2 = ispiti;
        long pocetak;
        long kraj;

        pocetak = System.currentTimeMillis();
        for(Ispit ispit : ispiti2){
            if(!ispit.getStudent().getJmbag().equals(student.getJmbag())){
                ispiti2.remove(ispit);
            }
        }
        kraj = System.currentTimeMillis();
        System.out.println("Potrebno vrijeme u ms za filtriranje ispita pomocu lambda izraza je: " + (kraj - pocetak));

        pocetak = System.currentTimeMillis();
        ispiti.removeIf(ispit -> ispit.getStudent().getJmbag().equals(student.getJmbag()));
        kraj = System.currentTimeMillis();
        System.out.println("Potrebno vrijeme u ms za filtriranje studenata pomocu for petlje je: " + (kraj - pocetak));
