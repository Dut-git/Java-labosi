...
public class ProfesorUnosController {
    ...
    public void dodajProfesora(){
        ...
        if(prazniTextFieldovi.isEmpty()){
            try {
                BazaPodataka.spremiNovogProfesora(new Profesor.ProfesorBuilder(CreateId(BazaPodataka.dohvatiProfesore()), imeProfesora, prezimeProfesora).saSifrom(sifraProfesora).saTitulom(titulaProfesora).build());
            } catch (BazaPodatakaException e) {
                throw new RuntimeException(e);
            }
